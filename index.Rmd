...
<br/>
<center>[Contextualisation et présentation de R et Rstudio](https://huguespecout.github.io/R_presentation_FR/#/) </center>  
<br/>
...
La fonction <span style="color:#8C001A">mean()</span> retourne NA car la série statistique présente une valeur manquante (NA). **L’argument na.rm (= FALSE par défaut) permet de ne pas prendre en compte les valeurs manquantes** :
...
En plus des fonction primitives, des fonctions que vous pouvez vous même créer, **il est très simple d'utiliser des fonctions mises à disposition par l'intermédiaire de package**. Pour cela, vous devez d'abord **installer le package et chargez la library** :
...
**L'objet <span style="color:#4B0082">sf</span> est en quelque sorte une table de données géographiques**. Il permet de gérer de l'information spatiale. Il s'agit tout simplement d'un dataframe qui stocke également des géométries.
...
De nombreuses de <span style="color:#8C001A">fonctions</span> (primitives ou non) peuvent être utilisées pour importer et exporter des données de différents formats.
...
Il existe de **très nombreux** package **pour réaliser des représentations graphiques** (point fort du langage R).
...
Dans l'exemple ci-dessous, nous utilisons la variable quantitative "*heures.tv*" :
...
mtcars$hp_classe <- cut(mtcars$hp, ma_discretisation,  include.lowest = TRUE, right = TRUE)
mtcars$hp_classe 

# Changement des nom de classe
levels(mtcars$hp_classe)<-c("A","B","C","D","E","F","G","H","I")
mtcars$hp_classe
