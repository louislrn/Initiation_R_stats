...
**L'objet sf est en quelque sorte une table de données géographique**. Il permet de gérer de l'information spatiale. Il s'agit tout simplement d'un dataframe qui stocke également des géométries.
...
Installer un <span style="color:#4B0082">package</span> en langage R :
...
Si vous souhaitez réaliser des traitements complexes sur des chaînes de caractères, vous pouvez utiliser la **librairie de référence** <span style="color:#8C001A">stringr</span> qui vous permettra de réaliser des opérations plus complexes. Exemples :
...
**Créez une nouvelle colonne** "***NOM***" dans votre objet sf, pour y stocker **le nom des communes en lettre CAPITALE**.
...
Plusieurs <span style="color:#8C001A">fonctions</span> primitives permettent de réaliser des graphiques :
...
La fonction <span style="color:#8C001A">plot()</span> permet l'affichage d'un objet sf (couche géographique) dans la fenêtre graphique.
...
Attention, le fonction <span style="color:#8C001A">plot()</span> réalise par défaut une représentation pour chaque variable de l'objet sf.     
**Pour afficher uniquement la géométrie, il est nécessaire d'utiliser la fonction** <span style="color:#8C001A">**st_geometry()**</span> du <span style="color:#4B0082">package<span> sf :
...
Il existe de **très nombreux** <span style="color:#4B0082">package</span> **pour réaliser des représentations graphiques** (oint fort du langage R).

**Le plus célèbre est** <span style="color:#8C001A">**ggplot2**</span>. Il permet de réaliser de très nombreux types de représentation graphique. Ce package est développé par Hadley Wickham (Rstudio) et **fonctionne avec un syntaxe particulière basée sur la** ***grammaire des graphiques***, théorisée par **Leland Wilkinson**. Exemple :
...
Dans un premier temps, nous utiliserons des données mises à disposition par la package <span style="color:#4B0082">questionr</span>, qui contient des variables qualitatives. **Installez** ce package, puis **chargez la librairie et les données associées** "*hdv2003*" :
