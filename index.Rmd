...
Ces deux fonctions sont directement accessibles en clic-bouton depuis l'interface Rstudio. Il suffit de cliquer sur l'objet dans l'onglet *Environnement* situé en haut à droite de l'IDE Rstudio (équivalent à *View()*) ou sur la flêche à gauche du nom de l'objet (équivalent à *str()*).
...
**Séléctionnez les communes isolées hors influence des pôles** (CATAEU2010 == 400) **de plus de 2000 habitants** en 2016 (P16_POP).
...
**Enregistrez le résultat de votre séléction par attribut dans un nouvel objet**.
...
**Vous devez assigner le résultat dans un objet pour enregister le nouvel ordre !**
...
**Les expressions régulières**, ou plus communément REGEX (contraction de *regular expression*) **permettent de représenter des modèles de chaînes de caractères. Ce sont des outils très puissants et très utilisés : on peut les retrouver dans de nombreux langages comme le PHP, MySQL, Javascript...** Si cet outil est très puissant, il est relativement difficile à appréhender au début car les expressions régulières peuvent prendre des formes complexe à interpreter. Ex :
...
**Sauvegardez votre objet sf joint aux données de recensement dans le repértoire de votre projet Rstudio**.
...
Enfin, **rechargez l'objet enregistré en dur** dans votre environnemnt Rstudio:
...
1) **Trouvez où est mis à dipsosition le package** <span style="color:#4B0082">**Rcade**</span>, développé par Romain Lesur  
2) **Installez-le**   
...
Avant de décrire, analyser une ou plusieurs variables statistiques, **il est indispensable de contrôler et d'explorer les données. Cela permet de détécter des incohérences et les caractèristiques des données étudiées, afin de correctement les traiter**.
...
Lorsque l'on étudie une variable, on s'intérresse rapidement à **la distribution de sa série statistique**.
...
A noter que **le mode** peut être également calculé pour les variables **qualitatives ordinales et nominales**, et **la médianne** peut être calculé pour les variables **qualitatives ordinales**.
...
<div class="alert alert-danger" role="alert">
**Le choix d'un méthode de discrétisation et du nombre de classes à un impact trés important sur l'interprétation de sa représentation visuelle. Cette <a href="http://mappemonde.mgm.fr/geovisu/119/" onclick="window.open(this.href); return false;"><b>application</b></a> [@geovisujegou] en est une belle demonstration**.</div>
<br/>
...
3) **Séléctionnez uniquement ces variables** et créez un nouveau dataframe :     
...
**Dans le cas où une variable qualitative ne présente que deux modalités**, la representation côte à côte des deux distributions permet  d'observer facilement leurs différences :
...
**Le calcul du coéfficient linéaire peut se faire avec la fonction <span style="color:#8C001A">cor.test()</span>** :
