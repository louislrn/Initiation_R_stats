...
```{r class.source="bg-danger", warning=FALSE, message=FALSE, eval=FALSE}
# Fonctions possibles
 test <-  read.csv("data/INSEE_COM_972.csv", header=TRUE, sep=",", stringsAsFactors=FALSE)
 test$CODGEO <- as.character(test$CODGEO) 
library(sf)
 test2 <- st_read("data/martinique.shp")
 
test3 <-  merge(test2, test, by.x="INSEE_COM" , by.y="CODGEO")

aggregate(data = test3, P16_POP ~ CATAEU2010, sum)

test3[order(test3$P16_POP, decreasing = FALSE), ]
test3$NOM <- toupper(test3$LIBGEO.x)

hist(test3$C16_POP15P_CS7, 
     breaks=12, 
     col="red",
     xlab= "mpg",
     ylab = "Fréquence",
...
Plusieurs packages permettent de faire des carte avec R, **le package** **<span style="color:#4B0082">mapsf</span>** **est spécialisé dans la production de cartes thématique et statistiques**.
...
6) **Créez une variable qualitative à partir des valeurs de la variable** ***TX_CS1_2016**

```{r class.source="bg-danger", warning=FALSE, message=FALSE, eval=FALSE}

### Deux solutions possibles :

### 1) Utiliser la fonction cut() 

### 2) Ou utiliser un opérateur relationnel... ;-)
...
7) **Réaliser uen jointure entre la couche géographique de la Martinique et le tableau de données INSEE**. 
...
<center><p class="objectif3"><a href="https://rzine.fr/"><img src="figures/Rzine_logo.png" width="170"/></a></p></center>
