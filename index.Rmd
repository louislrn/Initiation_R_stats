...
Avant de commencer, **créez un nouveau projet à l'emplacement de votre choix** ([cf. *Les projets*](#les-projets)). Puis, **téléchargez** [**ces données**](https://github.com/HuguesPecout/Initiation_R_stats/raw/main/data.zip), et décompressez l'archive à la racine de votre répertoire projet.
...
Il est possible d'importer et d'exporter des données de différents formats. Pour cela, une série de <span style="color:#8C001A">fonctions</span> (primitives ou non) peut être utilisées.
Quelques exemples :
...
read_sf()    # Données géographiques (shape, GeoJson...) = library(sf)
