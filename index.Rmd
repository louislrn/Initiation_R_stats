...
<br/>
<center>Diaporama bonus (équivalent "*Quelques fondamentaux*") : [Base du langage, packages, Rstudio et documentation](https://huguespecout.github.io/Intro_R_Rstudio_FR/#/)</center>
...
Pour discrétiser les valeurs de cette série statistique, utilisez la fonction <span style="color:#8C001A">mf_get_breaks()</span> du package mapsf :

```{r class.source="bg-info", class.output="bg-warning", warning=FALSE, message=FALSE}

# Construction d'un vecteur avec les bornes de chaque classe - mf_get_breaks() / library(mapsf)
# method :
...
library(mapsf)
my_discretisation <- mf_get_breaks(POP16_mtq$TX_0014 , breaks = "quantile")
my_discretisation
...
Cependant, pour réaliser toute sortes de cartes statistiques, vous pouvez n'utiliser qu'un seul package : '**<span style="color:#4B0082">mapsf</span>**'
...
propSymbolsChoroLayer(x = mtq, # sf object 
                      ...
                      method = "quantile", # discretization method (?mf_get_breaks)
...
```{r class.source="bg-info", class.output="bg-warning", warning=FALSE, message=FALSE, fig.width=5, fig.height=5}
