...
```{r class.source="bg-info", class.output="bg-warning", warning=FALSE, message=FALSE}
# Dans un dataframe, l'effectif correspond au nombre de lignes
nrow(mtcars)
```
...
# Exemple de fréquence d'un sous-effectif par rapport à l'effectif total
nrow(mtcars[mtcars$hp <= 100 ,]) / nrow(mtcars) * 100
...
# Déciles (quantiles de 10 en 10)
quantile(mtcars$hp, probs = seq(0,1,0.1), na.rm = TRUE)
...
quantile(mtcars$hp)[4] - quantile(mtcars$hp)[2]
...
# 1. Calculer les écarts à la moyenne
ecarts <- mtcars$hp - mean(mtcars$hp)

# 2. Extraire la valeur absolue
ecarts_abs <- abs(ecarts)

# 3. Calculer la moyenne
ecarts_abs_moy <- mean(ecarts_abs)
...
# 1. Calculer les écarts à la médiane
...
<div class="div_def"><p class="def">Indicateurs de la dispersion des valeurs des individus autour de la moyenne. La variance est la **moyenne des carrés des écarts à la moyenne.** L’écart-type est sa racine carrée. Ce sont des **paramètres de dispersion.** Attention, R calcule la variance en prenant en compte la **correction de Bessel**, en divisant les carrés des écarts à la moyenne par n-1 plutôt que par n, afin de simuler un échantillon.<br/>   
...
# 2. Elever ces écarts au carré
ecarts_sq <- ecarts * ecarts

# 3. Calculer la moyenne
variance <- mean(ecarts_sq)

# 4. Calculer l'écart-type
ecart_type <- sqrt(variance)

# Ou bien utiliser les fonctions de R...
variance <- var(mtcars$hp)
ecart_type <- sd(mtcars$hp)
...
variation <- ecart_type / mean(mtcars$hp) * 100
...
### Les valeurs centrales : Moyenne, Médiane, Mode
...
1- Relation entre **deux variables qualitatives** (4.2)      
2- Relation entre **deux variables quantitatives** (4.3)  
3- Relation entre **une variable qualitative et une variable quantitative** (4.4)      
...
<div class="div_def"><p class="def">Un test statistique est une procédure de décision entre deux hypothèses. Il s'agit d'une démarche consistant à rejeter ou à ne pas rejeter une hypothèse statistique, appelée hypothèse nulle et notée h0, en fonction d'un jeu de données (échantillon).</p>
</div>
...
--> [Le calcul du χ² et du degré de liberté ?](https://github.com/louislrn/Initiation_R_stats/raw/main/Khi2.xls)    
...
- la **significativité de la relation** (p-value), autrement dit le pourcentage de chances que les résultats observés ne soient pas dus au changement d’échelle (sur plusieurs simulations)
